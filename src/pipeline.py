"""
Master End-to-End Lunar Image Registration Pipeline.
Orchestrates preprocessing, phase/structural representation, feature extraction,
multi-scale pyramids, ANMS spatial optimization, sub-pixel refinement,
geometric model selection, warping, and comprehensive quality evaluation.
"""
...
from src.warping.resampler import ImageWarper
...
class RegistrationPipelineConfig:
    feature_method: str = "PHASE_STRUCTURAL"  # SIFT, AKAZE, PHASE_STRUCTURAL, LEARNED
...
class RegistrationOutput:
    ...
    def export_control_points_dataframe(self) -> pd.DataFrame:
        """
        Create structured pandas DataFrame of all control points.
        Uses zero-indexed floating-point pixel coordinates.
        """
        rows = []
...
class LunarRegistrationPipeline:
    ...
    def run(
        ...
        # 2. Illumination & Reliability
        lg = LogGaborPhaseCongruency()
        pc_src = lg.compute(src_norm)
        pc_ref = lg.compute(ref_norm)
        rel_src = compute_terrain_reliability(src_norm, pc_src.max_moment, src_shadow)
        rel_ref = compute_terrain_reliability(ref_norm, pc_ref.max_moment, ref_shadow)
        ...
        if len(candidates) < 4:
            raise RuntimeError(f"Found only {len(candidates)} candidate matches. Registration cannot proceed.")

        # 5. Spatial ANMS Optimization
        ...
        # 7. Independent Checkpoint Split (80% estimation, 20% validation)
        np.random.seed(42)
        ...
        # Physical Ground Error (Meters)
        ref_gsd = reference.metadata.gsd
